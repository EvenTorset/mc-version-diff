...
self.onmessage = async (event: MessageEvent<PeaksRequest>) => {
  ...
  const stride = Math.max(1, Math.floor(framesPerBucket / SAMPLES_PER_BUCKET))
  ...
  const onOutput = (data: AudioData) => {
    ...
    for (let channel = 0; channel < channels; channel++) {
      ...
      while (i < count) {
        ...
        const bucket = Math.min(buckets - 1, Math.floor(position / framesPerBucket))
        const slot = bucket * 2
        const end = Math.min(count, Math.ceil((bucket + 1) * framesPerBucket) - frame)
        let min = peaks[slot]
        let max = peaks[slot + 1]
        for (let at = i; at < end; at += stride) {
