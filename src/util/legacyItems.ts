...
export function legacyVariant(id: string, data?: number) {
  return VARIANTS[`${id}#${data ?? 0}`] ?? VARIANTS[id] ?? id
}
...
export function legacyAssetAlias(id: string): string | undefined {
  return ASSET_ALIASES[id]
}
